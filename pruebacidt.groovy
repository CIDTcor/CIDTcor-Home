...
preferences {
	page(name: "selectDevices", title: "Dispositivos", install: false, uninstall: true, nextPage: "selectPhrases") {
    	...
    	section("Dispositivos a controlar...") {
			...
			input "valve", "capability.valve", title: "Válvulas?", multiple: true, required: false
        }
...
def initialize() {
    ...
    subscribe(valve, "valve.closed", handler, [filterEvents: false])
    subscribe(valve, "valve.open", handler, [filterEvents: false])
    subscribe(valve, "valve", handler, [filterEvents: false])
}
...
def renderTile(data) {
	if (data.type == "weather"){
		...
		return """<div class="$data.type tile $data.active $data.closed" data-active="$data.active" data-closed="$data.closed" data-type="$data.type" data-device="$data.device" data-value="$data.value" data-level="$data.level" data-is-value="$data.isValue"><div class="title">$data.name</div></div>"""
...
def getDeviceData(device, type) {[tile: "device", active: isActive(device, type), closed: isClosed(device, type), type: type, device: device.id, name: device.displayName, value: getDeviceValue(device, type), level: getDeviceLevel(device, type), isValue: isValue(device, type)]}
...
def getActiveDeviceMap() {[lock: "unlocked", holiday: "on", "switch": "on", dimmer: "on", contact: "open", presence: "present", motion: "active", water: "wet", carbon_monoxide_detector: "clear", smoke: "clear", valve: "closed"]}
...
def isClosed(device, type) {
	def field = getDeviceFieldMap()[type]
	def value = device.respondsTo("currentValue") ? device.currentValue(field) : device.value
	def closed = value == getActiveDeviceMap()[type] ? "closed" : "open"
	closed
}
...
"""
	...
	<div class="contenedor">
		...
		<header>
			...
			<div class="titulo">
				Panel de control CORDOVA Smart v0.2b
			</div>
		</header>
		...
		<aside class="derecha">
			...
			<button id="Btn2">Salón Principal</button>
